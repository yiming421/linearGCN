...
def train(model, g, train_pos_edge, optimizer, neg_sampler, pred, feat_model):
    model.train()
    pred.train()
    ...
    for _, edge_index in enumerate(tqdm.tqdm(dataloader)):
        if args.maskinput:
            ...
            h = model(g_mask, feat_model())
            mask[edge_index] = 1
        else:
            h = model(g, feat_model())
...
def test(model, g, pos_test_edge, neg_test_edge, evaluator, pred, feat_model):
    model.eval()
    pred.eval()
...
def eval(model, g, pos_valid_edge, neg_valid_edge, evaluator, pred, feat_model):
    model.eval()
    pred.eval()
...
data = cp.ones(graph.number_of_edges()).reshape(-1)
row = cp.array(graph.edges()[0]).reshape(-1)
col = cp.array(graph.edges()[1]).reshape(-1)
A = sp.coo_matrix((data, (row, col)), shape=(graph.number_of_nodes(), graph.number_of_nodes()))
D = sp.diags(cp.squeeze(cp.array(A.sum(axis=1))))
L = D - A
st = time.time()
eigenvectors = linalg.eigsh(L, k=args.hidden)
print(time.time() - st, flush=True)

graph.ndata['feat'] = torch.tensor(eigenvectors[1], dtype=torch.float32)
graph.ndata['feat'] = graph.ndata['feat'][:, :args.hidden]
feat_model = feat(graph.ndata['feat']).to(device)
...
if args.model == 'GCN':
    model = GCN(graph.ndata['feat'].shape[1], args.hidden, args.norm, args.dp4norm, args.drop_edge, args.relu, args.prop_step, args.residual).to(device)
...
parameter = itertools.chain(model.parameters(), pred.parameters(), feat_model.parameters())
...
for epoch in range(args.epochs):
    loss = train(model, graph, train_pos_edge, optimizer, neg_sampler, pred, feat_model)
    losses.append(loss)
    if epoch % args.interval == 0 and args.step_lr_decay:
        adjustlr(optimizer, epoch / args.epochs, args.lr)
    valid_results = eval(model, graph, valid_pos_edge, valid_neg_edge, evaluator, pred, feat_model)
    valid_list.append(valid_results[args.metric])
    test_results = test(model, graph, test_pos_edge, test_neg_edge, evaluator, pred, feat_model)
